...
if (!$year || !$month) {
    header("Location: result_edit.php?error=" . urlencode("年度と月を指定してください。"));
    exit;
}
...
function redirectWithError($msg, $year, $month)
{
    ...
    header("Location: result_edit.php?year={$year}&month={$month}&error={$safeMsg}");
    exit;
}

$dbh = getDb();

$queryStatus = "SELECT id, status, hourly_rate FROM monthly_result WHERE year = :year AND month = :month";
$stmt = $dbh->prepare($queryStatus);
$stmt->execute([':year' => $year, ':month' => $month]);
$resultData = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$resultData) {
    redirectWithError("【{$year}年度 {$month}月】の概算実績は未登録です。", $year, $month);
}

$monthlyResultId = $resultData['id'];
$commonHourlyRate = (float)($resultData['hourly_rate'] ?? 0);
...
foreach ($offices as $office) {
    ...
    $queryTime = "
        ...
        FROM monthly_result_time 
        WHERE monthly_result_id = :monthly_result_id AND office_id = :office_id";
    $stmtTime = $dbh->prepare($queryTime);
    $stmtTime->execute([':monthly_result_id' => $monthlyResultId, ':office_id' => $officeId]);
...
header("Content-Disposition: attachment;filename=result_details_{$year}_{$month}.xlsx");
