...
// 確定済チェック
$queryStatus = "SELECT status FROM monthly_result WHERE year = :year AND month = :month";
$stmt = $dbh->prepare($queryStatus);
$stmt->execute([':year' => $year, ':month' => $month]);
$status = $stmt->fetchColumn();

if (!$status) {
    header("Location: result_edit.php?error=" . urlencode("【{$year}年度 {$month}月】の概算実績は未登録です。"));
    exit;
} elseif ($status !== 'fixed') {
    header("Location: result_edit.php?error=" . urlencode("【{$year}年度 {$month}月】の概算実績は未確定です。確定後に出力してください。"));
    exit;
}

// 労務費データ取得（result用）
$queryTime = "SELECT r.standard_hours, r.overtime_hours, r.transferred_hours, r.hourly_rate,
                     r.fulltime_count, r.contract_count, r.dispatch_count
              FROM monthly_result r
              WHERE r.year = :year AND r.month = :month";
$stmt = $dbh->prepare($queryTime);
$stmt->execute([':year' => $year, ':month' => $month]);
$timeData = $stmt->fetch(PDO::FETCH_ASSOC);

// 勘定科目ごとの合計金額取得（result明細を合計）
$queryAccount = "SELECT a.id AS account_id, a.name AS account_name, SUM(d.amount) AS total
                 FROM monthly_result_details d
                 JOIN monthly_result r ON d.result_id = r.id
...
$sheet->setTitle("概算実績_{$year}_{$month}");
...
header("Content-Disposition: attachment;filename=概算実績_{$year}_{$month}.xlsx");
