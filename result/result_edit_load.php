...
try {
    ...
    // 1. monthly_result の ID, status, 共通賃率, updated_atを取得
    $stmt = $dbh->prepare("SELECT id, status, hourly_rate, updated_at FROM monthly_result WHERE year = :year AND month = :month LIMIT 1");
    ...
    if ($result) {
        ...
        $response['monthly_result_id'] = $resultId;
        $response['status'] = $result['status'] ?? 'draft';
        $response['updated_at'] = $result['updated_at'];
        $response['common_hourly_rate'] = (float)($result['hourly_rate'] ?? 0);

        // 2. 営業所ごとの時間・人数データを取得
        // 外部キー名は monthly_result_id
        ...
        // 4. 詳細データ（経費）の取得
        // 外部キー名は result_id
        $detailStmt = $dbh->prepare("SELECT detail_id, amount FROM monthly_result_details WHERE result_id = :result_id");
        ...
        // 5. 収入データ
        // 外部キー名は result_id
        $revenueStmt = $dbh->prepare("SELECT revenue_item_id, amount FROM monthly_result_revenues WHERE result_id = :result_id");
