 ...
 */
function getResultStatusByYear($year, $dbh = null, $targetOfficeId = null)
{
    if (!$dbh) {
        $dbh = getDb();
    }
    ...
    if ($targetOfficeId > 0) {
        ...
        $sql = "
            SELECT 
                m.month, 
                t.status 
            FROM monthly_result m
            INNER JOIN monthly_result_time t ON m.id = t.monthly_result_id 
            WHERE m.year = ? 
            AND t.office_id = ?
        ";
        ...
        $sql = "SELECT month, status FROM monthly_result WHERE year = ?";
        $stmt = $dbh->prepare($sql);
        $stmt->execute([$year]);
    }
