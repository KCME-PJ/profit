 ...
 * @return int[] 年度の配列（昇順）
 */
function getAvailableResultYears($dbh = null)
{
    if (!$dbh) {
        $dbh = getDb();
    }

    $stmt = $dbh->query("SELECT DISTINCT year FROM monthly_result ORDER BY year ASC");
 ...
 * @return array
 */
function getResultFixedStatusByYear(int $year, PDO $dbh): array
{
    $statuses = array_fill(1, 12, 'draft');

    $stmt = $dbh->prepare("SELECT month, status FROM monthly_result WHERE year = :year");
