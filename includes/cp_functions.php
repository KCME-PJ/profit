...
function registerMonthlyCp(array $data, $dbh = null, array $userContext = [])
{
    ...
    if (!$monthly_cp_id) {
        $stmtCp = $dbh->prepare("INSERT INTO monthly_cp (year, month, status, created_at, updated_at) VALUES (?, ?, 'draft', NOW(), NOW())");
    ...
    insertCpTimeOnly([
        ...
        'hourly_rate' => $hourly_rate_common,
...
function updateMonthlyCp(array $data, PDO $dbh, array $userContext = [])
{
    ...
    $rateToSave = (isset($myTimeData['hourly_rate']) && $myTimeData['hourly_rate'] !== '')
        ? $myTimeData['hourly_rate']
        : $hourly_rate_common;
    ...
    // 親ヘッダー更新
    $dbh->prepare("UPDATE monthly_cp SET updated_at = NOW() WHERE id = ?")->execute([$monthly_cp_id]);
 ...
 * - Time: monthly_forecast_id使用, hourly_rate除外, type除外, LIMIT 1
 * - Details/Revenues: forecast_id使用, type除外
 */
function reflectToForecastSingleOffice($cpId, $officeId, $dbh)
{
    ...
    if (!$forecastId) {
        $stmtIns = $dbh->prepare("INSERT INTO monthly_forecast (year, month, status, created_at, updated_at) VALUES (?, ?, 'draft', NOW(), NOW())");
        $stmtIns->execute([$year, $month]);
    ...
    // Timeテーブル: hourly_rate, type を除外。LIMIT 1付与
    ...
    // Detailsテーブル: type 除外, forecast_id 使用
    ...
    // C. 賃率(hourly_rate)のコピー (親テーブル更新)
    $stmtRate = $dbh->prepare("SELECT hourly_rate FROM monthly_cp_time WHERE monthly_cp_id = ? AND office_id = ? AND type = 'cp' LIMIT 1");
    $stmtRate->execute([$cpId, $officeId]);
    $rate = $stmtRate->fetchColumn();
...
{
    $sql = "INSERT INTO monthly_cp_time (monthly_cp_id, office_id, standard_hours, overtime_hours, transferred_hours, hourly_rate, fulltime_count, contract_count, dispatch_count, type, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'cp', 'draft', NOW(), NOW())";
    $stmt = $dbh->prepare($sql);
    $stmt->execute([$data['monthly_cp_id'], $data['office_id'], $data['standard_hours'], $data['overtime_hours'], $data['transferred_hours'], $data['hourly_rate'], $data['fulltime_count'], $data['contract_count'], $data['dispatch_count']]);
}
function updateCpTimeOnly($id, $data, $dbh)
{
    $sql = "UPDATE monthly_cp_time SET standard_hours = ?, overtime_hours = ?, transferred_hours = ?, hourly_rate = ?, fulltime_count = ?, contract_count = ?, dispatch_count = ?, updated_at = NOW() WHERE id = ?";
    $stmt = $dbh->prepare($sql);
    $stmt->execute([$data['standard_hours'], $data['overtime_hours'], $data['transferred_hours'], $data['hourly_rate'], $data['fulltime_count'], $data['contract_count'], $data['dispatch_count'], $id]);
}
function deleteCpDetails($cpId, $officeId, $dbh)
{
    // 自拠点(office_id = ?) に加え、共通項目(office_id IS NULL) も削除対象に含める
