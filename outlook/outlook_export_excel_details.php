...
} elseif ($outlookData['status'] !== 'fixed') {
    redirectWithError("【{$year}年度 {$month}月】の月末見込みは未確定です。確定後に出力してください。", $year, $month);
}
...
foreach ($offices as $office) {
    ...
    // 営業所ごとの時間・人数データを取得 (Outlookデータのみ)
    $queryTime = "
        SELECT 
...
header("Content-Disposition: attachment;filename=outlook_details_{$year}_{$month}.xlsx"); // ファイル名を変更
