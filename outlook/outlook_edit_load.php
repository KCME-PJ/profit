...
try {
    ...
    // 1. monthly_outlook の ID, status, 共通賃率, updated_atを取得
    $stmt = $dbh->prepare("SELECT id, status, hourly_rate, updated_at FROM monthly_outlook WHERE year = :year AND month = :month LIMIT 1");
    $stmt->execute(['year' => $year, 'month' => $month]);
    $outlook = $stmt->fetch(PDO::FETCH_ASSOC);

    // 初期レスポンス構造
    $response = [
        'monthly_outlook_id' => 0,
        'offices' => [],
        'details' => [],
        'revenues' => [],
        'status' => 'none',
    ...
    if ($outlook) {
        ...
        $response['monthly_outlook_id'] = $outlookId;
        $response['status'] = $outlook['status'] ?? 'draft';
        $response['updated_at'] = $outlook['updated_at'];
        $response['common_hourly_rate'] = (float)($outlook['hourly_rate'] ?? 0);

        // 2. 営業所ごとの時間・人数データを取得
        // 外部キー名は monthly_outlook_id
        ...
        // 4. 詳細データ（経費）の取得
        // 外部キー名は outlook_id
        $detailStmt = $dbh->prepare("SELECT detail_id, amount FROM monthly_outlook_details WHERE outlook_id = :outlook_id");
        ...
        // 5. 収入データ
        // 外部キー名は outlook_id
        $revenueStmt = $dbh->prepare("SELECT revenue_item_id, amount FROM monthly_outlook_revenues WHERE outlook_id = :outlook_id");
