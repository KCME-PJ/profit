...
try {
    ...
    $revenueQuery = "SELECT 
                        ...
                        i.id AS item_id, i.name AS item_name, i.note AS item_note, 
                        i.office_id AS item_office_id, o.name AS item_office_name
    ...
    foreach ($revenueDetails as $row) {
        ...
        if (!isset($revenues[$categoryId])) {
            $revenues[$categoryId] = ['name' => $row['category_name'], 'items' => []];
        }
    ...
    $accountsQuery = "SELECT 
                        ...
                        d.id AS detail_id, d.name AS detail_name, d.note AS detail_note,
                        d.office_id AS detail_office_id, o.name AS detail_office_name
                    FROM accounts a
    ...
    foreach ($accountsDetails as $row) {
        ...
        if (!isset($accounts[$accountId])) {
            $accounts[$accountId] = ['name' => $row['account_name'], 'details' => []];
        }
...
<head>
    ...
    <script src="../js/outlook_edit_head.js" defer></script>
</head>
...
<body data-user-role="<?= htmlspecialchars($userRole) ?>">
    ...
    <div class="container mt-2">
        ...
        <form id="mainForm" action="outlook_update.php" method="POST">
            <div class="row mb-3">
                <div class="col-md-2">
                    <h4 class="mb-2" id="editTitle">月末見込み 編集</h4>
                </div>
                <div class="col-md-6">
            ...
            <div class="info-box">
                ...
                <div class="row align-items-end mb-2">
                    <input type="hidden" id="monthlyOutlookId" name="monthly_outlook_id">
                    <input type="hidden" id="olStatus" name="status" value="">
                ...
                <?php if ($isAdmin): ?>
                    ...
                    <div id="adminParentControls" style="display:none;" class="d-inline-flex align-items-center gap-2">
                        <button type="button" class="btn btn-primary btn-sm" id="outlookParentFixConfirmBtn" disabled data-bs-toggle="modal" data-bs-target="#parentFixModal">
                            <i class="bi bi-check2-all"></i> 月次確定 (Parent Fix)
                        </button>
                        <button type="button" class="btn btn-outline-danger btn-sm" id="outlookParentUnlockConfirmBtn" style="display:none;" data-bs-toggle="modal" data-bs-target="#parentUnlockModal">
                ...
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 150px;">金額（見込み）</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-warning" id="outlookRejectConfirmBtn">はい、差し戻す</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="parentFixModal" tabindex="-1" aria-labelledby="parentFixModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">キャンセル</button>
                            <button type="button" class="btn btn-primary" id="outlookParentFixConfirmBtn">月次確定する</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="parentUnlockModal" tabindex="-1" aria-labelledby="parentUnlockModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">キャンセル</button>
                            <button type="button" class="btn btn-danger" id="outlookParentUnlockConfirmBtn">ロック解除する</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script src="../js/outlook_edit_body.js"></script>
</body>
