...
// 確定済チェック
$queryStatus = "SELECT status FROM monthly_cp WHERE year = :year AND month = :month";
$stmt = $dbh->prepare($queryStatus);
$stmt->execute([':year' => $year, ':month' => $month]);
$status = $stmt->fetchColumn();

if (!$status) {
    header("Location: cp_edit.php?error=" . urlencode("【{$year}年度 {$month}月】のCPは未登録です。"));
    exit;
} elseif ($status !== 'fixed') {
    header("Location: cp_edit.php?error=" . urlencode("【{$year}年度 {$month}月】のCPは未確定です。確定後に出力してください。"));
    exit;
}

// 労務費データ取得
$queryTime = "SELECT t.standard_hours, t.overtime_hours, t.transferred_hours, t.hourly_rate, t.fulltime_count, t.contract_count, t.dispatch_count
              FROM monthly_cp_time t
              JOIN monthly_cp cp ON t.monthly_cp_id = cp.id
              WHERE cp.year = :year AND cp.month = :month AND t.type = 'cp'";
...
$queryAccount = "SELECT a.id AS account_id, a.name AS account_name, SUM(d.amount) AS total
                 ...
                 JOIN monthly_cp cp ON d.monthly_cp_id = cp.id
...
$sheet->setTitle("CP_{$year}_{$month}");
...
header("Content-Disposition: attachment;filename=cp_{$year}_{$month}.xlsx");
