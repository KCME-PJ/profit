...
// 確定済チェック
$queryStatus = "SELECT status FROM monthly_forecast WHERE year = :year AND month = :month";
$stmt = $dbh->prepare($queryStatus);
$stmt->execute([':year' => $year, ':month' => $month]);
$status = $stmt->fetchColumn();

if (!$status) {
    header("Location: forecast_edit.php?error=" . urlencode("【{$year}年度 {$month}月】の見通しは未登録です。"));
    exit;
} elseif ($status !== 'fixed') {
    header("Location: forecast_edit.php?error=" . urlencode("【{$year}年度 {$month}月】の見通しは未確定です。確定後に出力してください。"));
    exit;
}

// 労務費データ取得（forecast用）
$queryTime = "SELECT f.standard_hours, f.overtime_hours, f.transferred_hours, f.hourly_rate,
                     f.fulltime_count, f.contract_count, f.dispatch_count
...
$sheet->setTitle("見通し_{$year}_{$month}");
...
header("Content-Disposition: attachment;filename=見通し_{$year}_{$month}.xlsx");
