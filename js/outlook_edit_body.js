document.addEventListener('DOMContentLoaded', function () {
    ...
    function captureCurrentOfficeTime(oid) {
        ...
        for (const id in officeTimeDataLocal) {
            ...
            // hourly_rate は monthly_outlook の親テーブルで管理されるため、ここで全営業所の子データにも格納しておく
    ...
    function prepareAndSubmitForm(action) {
        ...
        // monthly_outlook_time にデータを入れるため、親の monthly_outlook の項目は空にする
        if (form) {
            // 賃率以外の親テーブル項目をクリア
            ...
            // 賃率のみは共通値としてmonthly_outlookに保存されるため、そのまま残す
        }
    ...
    // --- 月/営業所選択時のデータ読み込み処理 ---
    // ---------------------------------------------
    window.loadOutlookData = function (year, month, officeId) {
    ...
    monthSelect.addEventListener('change', function () {
        ...
        const officeId = officeSelect.value; // officeIdは常に現在の選択値を使用
        if (!year || !month || !officeId) return;
        window.loadOutlookData(year, month, officeId);
    });
