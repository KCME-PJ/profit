/**
 * outlook_edit_body.js
 * forecast/planのロジックを移植し、1000件対応とUI改善を適用
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getFormDataString() {
        ...
        const ignoreIds = [
            ...
            'outlookMode',  // actionタイプのため除外
            'olStatus'      // ステータス自体は比較対象外
        ];
    ...
    function prepareAndSubmitForm(action) {
        ...
        if (document.getElementById('outlookMode')) {
            document.getElementById('outlookMode').value = action;
        }
    ...
    function resetInputFields() {
        ...
        officeTimeDataLocal = {};
        if (document.getElementById('outlookId')) document.getElementById('outlookId').value = '';
    ...
    if (document.getElementById('outlookFixConfirmBtn')) {
        ...
        monthSelect.addEventListener('change', function () {
            ...
            fetch(`outlook_edit_load.php?year=${year}&month=${month}`)
                ...
                .then(data => {
                    ...
                    if (document.getElementById('outlookId')) {
                        document.getElementById('outlookId').value = data.outlook_id ?? '';
                    }

                    // ステータスをhiddenにセット (存在すれば)
                    if (document.getElementById('olStatus')) {
                        document.getElementById('olStatus').value = data.status ?? '';
                    }
