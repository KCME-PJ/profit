/**
 * cp_edit_body.js (修正版: 賃率隠しフィールド対応)
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getFormDataString() {
        ...
        const ignoreIds = [
            ...
            'cpMode',       // actionタイプのため除外
            'cpStatus'      // ステータス自体は比較対象外
        ];
        ...
        monthSelect.addEventListener('change', function () {
            ...
            fetch(`cp_edit_load.php?year=${year}&month=${month}`)
                ...
                .then(data => {
                    ...
                    // ステータスをhiddenにセット
                    if (document.getElementById('cpStatus')) {
                        document.getElementById('cpStatus').value = data.status ?? '';
                    }
